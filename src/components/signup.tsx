...
import { Shield, User, Mail, Lock, Briefcase, GraduationCap, Building } from "lucide-react"
...
export function Signup() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formData = {
      ...
      email,
      password,
      ...(accountType === 'therapist' ? { certification, specialization } : { organizationType, organizationName })
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-100 to-white flex flex-col">
      ...
      <main className="flex-grow flex items-center justify-center px-4 py-12">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <Tabs defaultValue="therapist" onValueChange={setAccountType} className="w-full">
          ...
          <CardFooter>
            <div className="text-sm text-gray-600 text-center w-full">
              ...
              <Link href="/login" className="text-green-600 hover:bg-green-700 font-semibold">
                Log in
              </Link>
            </div>
          </CardFooter>
      ...
      <footer className="bg-green-800 text-white p-6">
        <p className="text-center">&copy; {new Date().getFullYear()} Gamesafe. All rights reserved.</p>
      </footer>
    </div>
  )
}